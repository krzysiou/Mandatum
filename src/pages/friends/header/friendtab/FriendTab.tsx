...
//set name of friend
async function setFriendName(
  setName: React.Dispatch<React.SetStateAction<string>>,
  id: string
) {
  axios
    .post(
      'http://localhost:3001/users/get/username',
      { id: id },
...
//set values
async function setStartupValues(
  id: string,
...
async function pin(
  ...
  if (label === 'pin') {
    axios
      .post(
        'http://localhost:3001/users/pinned/add',
        { username: name },
        { headers: { Authorization: `Bearer ${getCookie('accessToken')}` } }
      )
      .then(function (response) {
    ...
    axios
      .post(
        'http://localhost:3001/users/pinned/remove',
        { username: name },
        { headers: { Authorization: `Bearer ${getCookie('accessToken')}` } }
      )
      .then(function (response) {
...
async function removeFriend(
  ...
  axios
    .post(
      'http://localhost:3001/users/remove',
      { id: id },
      { headers: { Authorization: `Bearer ${getCookie('accessToken')}` } }
    )
    .then(function (response) {
...
//handles user click
export async function handleClick(id: string): Promise<void> {
  axios
    .post(
      'http://localhost:3001/users/click',
      { id: id },
      { headers: { Authorization: `Bearer ${getCookie('accessToken')}` } }
    )
    .then(function (response) {
...
export function FriendTab({ id, decodeToken, setUser }: Props): JSX.Element {
  ...
  return (
    <div className="friend-tab">
      <div className="friend-name cont" onClick={() => handleClick(id)}>
        {name}
      </div>
      <div className="friend-pin-button cont">
        <Button
          label={label}
          width="3rem"
          onPress={() => pin(setLabel, label, name)}
        />
        <Button
          label="X"
          width="2rem"
          onPress={() => removeFriend(id, decodeToken, setUser)}
        />
      </div>
    </div>
  );
